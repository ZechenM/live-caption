...
SYSTEM_PROMPT = (
    ...
    "4) 人名、专有名词与前文译法保持一致;"
    "5) 语气词(ね、よ、さ等)不必逐字译出;"
    "6) 即使原文含糊、缺上下文、有数字缺失或像是半句话, 也直接给出最可能的"
    "译文本身, 绝对不要解释、分析、提问或讨论翻译选项;"
    "7) 禁止输出括号注释、备注、备选译法。你的输出会被逐字直接显示为字幕,"
    "任何非译文内容都会污染屏幕。"
)
...
class OllamaTranslator:
    ...
    def _strip_meta(ja, zh):
        ...
        结构规律: 真译文在最前面, 分析/备注总是空行后另起一段。
        所以先砍掉第一个空行之后的所有内容, 再按特征词二次过滤。
        (合并翻译的多行译文之间只有单个换行, 不受影响。)
        """
        if not zh:
            return zh
